...
@group(1) @binding(1) var<storage, read> splatCovariances: array<f32>;
@group(1) @binding(2) var<storage, read> visibleSplatIndices: array<u32>;
@group(1) @binding(3) var<storage, read> splatIds: array<u32>;

fn project_covariance_2d(worldCenter: vec3<f32>, cov3d: array<f32, 6>) -> vec3<f32> {
...
fn ellipse_offset(cov2d: vec3<f32>, corner: vec2<f32>) -> vec2<f32> {
  ...
  return axis0 * sqrt(lambda0) * 3.0 * corner.x + axis1 * sqrt(lambda1) * 3.0 * corner.y;
}
...
fn vsMain(
  ...
  let ndcOffset = ellipse_offset(project_covariance_2d(worldCenter, covariance), corner);
  ...
  output.splatId = splatIds[splatIndex] + 1u;
  return output;
}

@fragment
fn fsMain(input: VertexOutput) -> @location(0) u32 {
  if (dot(input.localPos, input.localPos) > 1.0) {
    discard;
  }
